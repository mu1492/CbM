...
{   
    ...
    setAccelerationRange( ACCELERATION_RANGE_2G );
    updateLsbToG();
...
//!************************************************************************
bool Adxl355::calculateLogisticResponse
    (
...
//!************************************************************************
bool Adxl355::checkOffsetRealRange
    (
...
//!************************************************************************
bool Adxl355::compensateOffsetTemperature
    (
...
//!************************************************************************
bool Adxl355::convertOffsetRealToInt
    (
    ...
    )
{
    ...
    if( status )
    {
        ...
        status = readByteReg( ADXL355_REG_POWER_CTL, &powerCtlReg );
        ...
        if( status )
        {
            ...
            status = writeByteReg( ADXL355_REG_POWER_CTL, powerCtlReg );
        }
    }
...
//!************************************************************************
bool Adxl355::enableActivityAxis
    (
    ...
    )
{
    ...
    if( status )
    {
        ...
        status = readByteReg( ADXL355_REG_ACT_EN, &activityEnReg );
        ...
        if( status )
        {
            ...
            status = writeByteReg( ADXL355_REG_ACT_EN, activityEnReg );
        }
    }
...
//!************************************************************************
bool Adxl355::enableExternalClk
    (
    ...
    )
{
    ...
    {
        uint8_t syncReg = 0;
        status = readByteReg( ADXL355_REG_SYNC, &syncReg );
        ...
        if( status )
        {
            ...
            status = writeByteReg( ADXL355_REG_SYNC, syncReg );
        }
    }
...
//!************************************************************************
bool Adxl355::enableInterruptSource
    (
    ...
    )
{
    ...
    if( status )
    {
        ...
        status = readByteReg( ADXL355_REG_INT_MAP, &interruptMapReg );
        ...
        if( status )
        {
            ...
            status = writeByteReg( ADXL355_REG_INT_MAP, interruptMapReg );
        }
    }
...
//!************************************************************************
bool Adxl355::enableSelfTestMode
    (
    const bool aEnterSelfTest       //!< true for entering self test mode
    )
{
    uint8_t selftestReg = 0;
    bool status = readByteReg( ADXL355_REG_SELF_TEST, &selftestReg );
    ...
    if( status )
    {
        ...
        if( aEnterSelfTest != mSelfTestMode )
        {
            ...
            status = writeByteReg( ADXL355_REG_SELF_TEST, selftestReg );
...
//!************************************************************************
bool Adxl355::enableStandbyMode
    (
    const bool aEnterStandby    //!< true for entering standby
    )
{
    uint8_t powerCtlReg = 0;
    bool status = readByteReg( ADXL355_REG_POWER_CTL, &powerCtlReg );
...
//!************************************************************************
bool Adxl355::getAccelerationOnAxis
    (
    ...
    )
{
    ...
    if( status )
    {
        *aAcceleration = 0;
        ...
        {
            case AXIS_X:
                status = readByteArrayReg( ADXL355_REG_XDATA3, LENGTH, bArray );
                break;

            case AXIS_Y:
                status = readByteArrayReg( ADXL355_REG_YDATA3, LENGTH, bArray );
                break;

            case AXIS_Z:
                status = readByteArrayReg( ADXL355_REG_ZDATA3, LENGTH, bArray );
                break;
        ...
        if( status )
        {
            status = compensateOffsetTemperature( aAxis, aAcceleration );
        }
    }
...
//!************************************************************************
bool Adxl355::getAccelerationRange
    (
    ...
    )
{
    ...
    if( status )
    {
        *aRange = ACCELERATION_RANGE_2G;
        uint8_t rangeReg = 0;
        status = readByteReg( ADXL355_REG_RANGE, &rangeReg );

        if( status )
        {
            const uint8_t RANGE_MASK = RANGE_PM8G;
...
//!************************************************************************
bool Adxl355::getAccelerationsOnAllAxes
    (
...
//!************************************************************************
double Adxl355::getCrossAxisSensitivity() const
{
    ...
    )
{
    ...
    if( status )
    {
        *aData = 0;
        ...
        status = readByteReg( ADXL355_REG_FIFO_DATA, &fifoDataReg );
...
//!************************************************************************
bool Adxl355::getFifoSamplesSize
    (
    ...
    )
{
    ...
    if( status )
    {
        *aSize = 0;
        ...
        status = readByteReg( ADXL355_REG_FIFO_SAMPLES, &fifoSamplesReg );
...
//!************************************************************************
bool Adxl355::getFifoValidSamplesCount
    (
    ...
    )
{
    ...
    if( status )
    {
        *aNumber = 0;
        ...
        status = readByteReg( ADXL355_REG_FIFO_ENTRIES, &fifoEntriesReg );
...
//!************************************************************************
bool Adxl355::getInclinationStepAngle
    (
    const Axis   aAxis,                     //!< axis
...
//!************************************************************************
double Adxl355::getOdrDelay() const
{
...
//!************************************************************************
double Adxl355::getOdrFrequency() const
{
    return mOdr.freq;
}
...
//!************************************************************************
double Adxl355::getOdrHpfCorner() const
{
...
//!************************************************************************
double Adxl355::getOdrLpfCorner() const
{
...
//!************************************************************************
//! Get the ADXL355 chip revision ID
//!
//! @returns The chip revision ID
//!************************************************************************
uint8_t Adxl355::getRevId() const
{
    return mRevId;
}
...
//!************************************************************************
bool Adxl355::getTemperatureOff
    (
...
//!************************************************************************
bool Adxl355::getTemperatureValue
    (
    ...
    )
{
    ...
    if( status )
    {
        ...
        *aTemperature = TEMP_INVALID_C;
        ...
        {
            uint16_t tempRaw = 0;
            readWordReg( ADXL355_REG_TEMP2, &tempRaw );
            tempRaw &= 0x0fff;
...
//!************************************************************************
bool Adxl355::getVibrationRectificationOffset
    (
    ...
    )
{
    ...
    if( status )
    {
        status = readByteReg( ADXL355_REG_DEVID_AD, &readByte );
    ...
    if( status )
    {
        status = readByteReg( ADXL355_REG_DEVID_MST, &readByte );
    ...
    if( status )
    {
        status = readByteReg( ADXL355_REG_PART_ID, &readByte );

        if( status )
        {
            const uint8_t PART_ID = 0xed;     //!< Analog Devices part ID (ADXL355)
    ...
    if( status )
    {
        status = readByteReg( ADXL355_REG_REV_ID, &readByte );
    ...
    if( status )
    {
        status = readByteReg( ADXL355_REG_POWER_CTL, &readByte );
...
//!************************************************************************
bool Adxl355::isActivityDetected
    (
    ...
    ) const
{
    ...
    if( status )
    {
        *aDetection = false;
        uint8_t statusReg = 0;
        status = readByteReg( ADXL355_REG_STATUS, &statusReg );
...
//!************************************************************************
bool Adxl355::isExternalClkEnabled
    (
...
bool Adxl355::reset()
{
    ...
    if( mRevId > 1 )
    {
        status = writeByteReg( ADXL355_REG_RESET, RESET_CODE );
    }
    else
    {
        const uint8_t REG_SHADOW_COUNT = 1 + ADXL355_REG_SHADOW_LAST - ADXL355_REG_SHADOW_FIRST;
        ...
        {
            status = readByteArrayReg( ADXL355_REG_SHADOW_FIRST, REG_SHADOW_COUNT, beforeShadowRegArray );
            ...
            if( status )
            {
                ...
                if( status )
                {
                    ...
                    status = readByteArrayReg( ADXL355_REG_SHADOW_FIRST, REG_SHADOW_COUNT, afterShadowRegArray );
...
//!************************************************************************
bool Adxl355::setAccelerationRange
    (
    ...
    )
{
    ...
    if( status )
    {
        ...
        if( status )
        {
            ...
            status = writeByteReg( ADXL355_REG_RANGE, rangeReg );
    ...
    )
{
    ...
    if( status )
    {
        status = writeByteReg( ADXL355_REG_ACT_COUNT, aNumber );
    }
...
//!************************************************************************
bool Adxl355::setActivityThreshold
    (
...
//!************************************************************************
bool Adxl355::setFifoSamplesSize
    (
    ...
    )
{
    ...
    if( status )
    {
        ...
        if( status )
        {
            ...
            status = writeByteReg( ADXL355_REG_FIFO_SAMPLES, fifoSamplesReg );
    ...
    )
{
    ...
    {
        uint8_t filterReg = 0;
        status = readByteReg( ADXL355_REG_FILTER, &filterReg );

        if( status )
        {
            filterReg &= ~FILTER_HPF_CORNER;
            filterReg |= static_cast<uint8_t>( aHpfSetting );
            status = writeByteReg( ADXL355_REG_FILTER, filterReg );
...
//!************************************************************************
bool Adxl355::setInterruptPinsPolarity
    (
...
//!************************************************************************
bool Adxl355::setTemperatureOff
    (
...
//!************************************************************************
bool Adxl355::storeActivityThreshold()
{
    ...
    if( status )
    {
        ...
        if( status )
        {
            status = writeWordReg( ADXL355_REG_ACT_THRESH_H, u16ActivityThd );
        }
    }
    ...
    )
{
    ...
    if( status )
    {
        ...
        if( status )
        {
            ...
            {
                case AXIS_X:
                    status = writeWordReg( ADXL355_REG_OFFSET_X_H, intOffset );
                    break;

                case AXIS_Y:
                    status = writeWordReg( ADXL355_REG_OFFSET_Y_H, intOffset );
                    break;

                case AXIS_Z:
                    status = writeWordReg( ADXL355_REG_OFFSET_Z_H, intOffset );
                    break;
...
//! @returns nothing
//!************************************************************************
void Adxl355::updateHpfBySelection()
{
...
//! @returns nothing
//!************************************************************************
void Adxl355::updateOdrBySelection()
{
    for( uint8_t i = 0; i < ODR_SETTINGS_COUNT; i++ )
    {
